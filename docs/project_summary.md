...
### 1. 实时市场监控与多维数据展示 (Dashboard & DataManager)
- **多渠道数据接入**: 通过 WebSocket (Kline 数据) 保证数据的低延迟实时性，结合 REST API 作为后备和拉取持仓量（Open Interest, OI）、外围指标（如资金费率 Funding Rate）的手段。
...
- **现成策略库**: 涵盖简单趋势（Simple MA/Momentum）、复合策略（Strong Breakout 强突破、Trend Confirmation 趋势确认、Capital Inflow 资金流入）以及特殊统计模型策略（RSRS 等）。
...
### 优点 (Pros)
1. **纯粹的前端工程化优势**: 通过 Hooks 将繁杂的逻辑拆分（例如 WebSocket 的封装 `useKlineWebSocket`、警告系统的封装），极大提升了代码复用性和可维护性。
...
3. **API 限流风险**: 在全市场同时发起数据请求（无论 REST 还是 WebSocket 退化）容易撞到 Binance API 的权重阈值限制，项目内的 `apiRetry` 虽能缓解，但在极端行情拥堵时极易丢失数据。
...
- 后端能保证 24/7 不间断扫描，并在条件触发时接入外部通知（如 Telegram Bot 甚至电话语音），形成真正的监控中心。
