...
import WorkspaceViews, { type AppTab } from '@/components/workspace/WorkspaceViews';
...
import { resolveWorkspaceMarketPolicy } from '@/lib/workspaceMarketPolicy';
...
export default function AppWorkspace() {
  ...
  const {
    watchlists,
    activeWatchlist,
    storageReady: watchlistStorageReady,
    addWatchlist,
    updateWatchlistName,
    removeWatchlist,
    setActiveWatchlist,
  ...
  }, [dashboardWatchlistId, watchlistStorageReady, watchlists]);
  ...
  const marketPolicy = resolveWorkspaceMarketPolicy(activeTab);
  const shouldRunLiveMarketRequests = marketPolicy.runLiveMarketRequests;
  const shouldRunDeferredIndicatorRequests = marketPolicy.runDeferredIndicatorRequests;
  const shouldRunLeaderboardRequests = marketPolicy.runLeaderboardRequests;
  const shouldRunHeavyMarketRequests = marketPolicy.runHeavyMarketRequests;
  const heavyMarketEndpoint = marketPolicy.heavyMarketEndpoint;
  ...
  return (
    <WorkspaceViews
      ...
      showMarketConnectionAlert={showMarketConnectionAlert}
      showEmptyMarketAlert={shouldRunLiveMarketRequests && !marketLoading && !marketError && processedData.length === 0}
      showOpenInterestUnavailableAlert={showOpenInterestUnavailableAlert}
      dashboardProps={{
        processedData, openInterestFrames: oiFrameData, onSymbolClick: handleSymbolClick,
        demoMode: isDemoMode, watchlists, selectedWatchlistId: dashboardWatchlistId,
        onSelectWatchlist: setDashboardWatchlistId,
      }}
      leaderboardProps={{ data: processedData, openInterestFrames: oiFrameData, onSymbolClick: handleSymbolClick }}
      macroProps={{ onSymbolClick: handleSymbolClick }}
      watchlistsProps={{
        marketData: processedData, watchlists, activeWatchlistId: activeWatchlist?.id ?? null,
        onSymbolClick: handleSymbolClick, onSelectWatchlist: setActiveWatchlist,
        onCreateWatchlist: addWatchlist, onDeleteWatchlist: removeWatchlist,
        onRenameWatchlist: updateWatchlistName, onAddSymbol: addSymbol, onRemoveSymbol: removeSymbol,
      ...
      strategyProps={{
        signals: visibleSignals, dismissSignal, clearAllSignals, onSymbolClick: handleSymbolClick,
        marketDataStatus: strategyMarketDataStatus, scanDiagnostics,
      }}
      tradingProps={{
        strategyParameterOverrides,
        onStrategyParameterOverridesChange: setStrategyParameterOverrides,
      }}
      chartProps={{ symbol: selectedSymbol, isOpen: isChartOpen, onClose: handleCloseChart }}
    />
  );
}
